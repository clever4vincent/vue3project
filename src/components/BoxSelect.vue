...
const isWithinRange = (dom, top, bottom, left, right) => {
  ...
  return !(eleRect.top > bottom || eleRect.bottom < top || eleRect.right < left || eleRect.left > right);
};
...
export default {
  ...
  /**
   * @member props
   * @property  {String} [node] 要框选的元素,能够是元素名,也能够是class名, 也能够是id名
  ...
  setup(props, { emit }) {
    ...
    const getAreaWithinElements = () => {
      ...
      elements.map((item, index) => {
        ...
        withinRange && selectedElementsIndex.push(index);
      });
      ...
      //   const withinRange = isWithinRange(item, top, bottom, left, right);
      //   withinRange && classOperation(item, "contains", props.selectedClass) && classOperation(item, "remove", props.selectedClass);
      // });

      // unselectedElements.map((item) => isWithinRange(item, top, bottom, left, right) && classOperation(item, "add", props.selectedClass));
