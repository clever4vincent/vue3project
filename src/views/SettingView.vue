<template>
  <div class="page">
    ...
    <van-cell-group>
      ...
      <van-cell title="账号信息存储" is-link @click="saveInfo"></van-cell>
      <van-cell title="账号信息读取" is-link @click="readInfo"></van-cell>
...
<script setup>
import { useThemeStore, useAccountStore, useConditionStore } from "../stores";
import { useDark, useToggle } from "@vueuse/core";
import { saveAccountThrid, getAccountThrid } from "@/api";
...
const saveInfo = () => {
  showConfirmDialog({
    ...
    .then(() => {
      // const accountStore = useAccountStore();
      let condition = {
        conditionGroups: toRaw(conditionStore.conditionGroups),
        equipmentModifys: toRaw(conditionStore.equipmentModifys),
      };

      saveAccountThrid({ name: "a6669852", account: toRaw(accountStore.$state), condition }).then((res) => {
...
const readInfo = () => {
  showConfirmDialog({
    ...
    .then(() => {
      // const accountStore = useAccountStore();
      getAccountThrid({ name: "a6669852" }).then((res) => {
        // console.log(res);
        // accountStore.
        accountStore.$patch(res.account);
        conditionStore.$patch(res.condition);
        showSuccessToast("读取成功");
      });
    })
    .catch(() => {
      console.log("cancel");
